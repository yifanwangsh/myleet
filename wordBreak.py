...
def wordBreak(s,wordDict):
    ...
    for word in wordDict:
        if s.find(word)>=0:
            fragments=s.split(word)
            checklist=[]
            for crack in fragments:
                checklist.append(wordBreak(crack,wordDict))
            if all(checklist):
                return True
    return False              
...
class TestMethod(unittest.TestCase):
    def test1(self):
        ...
        self.assertEqual(False,wordBreak(s,wordDict))

    def test2(self):
        s="leetcode"
        wordDict=["leet","code"]  
        self.assertEqual(True,wordBreak(s,wordDict))
    ...
    def test5(self):
        s="applecatsanddog"
        wordDict=["apple","cats","sand","dog","cat"]
        self.assertEqual(True,wordBreak(s,wordDict))
